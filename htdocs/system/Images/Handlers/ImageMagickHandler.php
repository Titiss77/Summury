...
class ImageMagickHandler extends BaseHandler
{
    ...
    {
        $angle = 'horizontal' === $direction ? '-flop' : '-flip';
    ...
    protected function process(string $action, int $quality = 100): array
    {
        ...
        $cmd = $this->config->libraryPath;
        $cmd .= '-version' === $action ? ' '.$action : ' -quality '.$quality.' '.$action;
        ...
        if (isset($options['hAlign'], $options['vAlign'])) {
            switch ($options['hAlign']) {
                case 'left':
                    ...
                    $gravity = 'top' === $options['vAlign'] ? 'NorthWest' : 'West';
                    if ('bottom' === $options['vAlign']) {
                        $gravity = 'SouthWest';
                        $yAxis = $options['vOffset'] - $options['padding'];
                    }
                ...
                case 'center':
                    $xAxis = $options['hOffset'] + $options['padding'];
                    $yAxis = $options['vOffset'] + $options['padding'];
                    $gravity = 'top' === $options['vAlign'] ? 'North' : 'Center';
                    if ('bottom' === $options['vAlign']) {
                        $yAxis = $options['vOffset'] - $options['padding'];
                        $gravity = 'South';
                    }
                ...
                case 'right':
                    $xAxis = $options['hOffset'] - $options['padding'];
                    $yAxis = $options['vOffset'] + $options['padding'];
                    $gravity = 'top' === $options['vAlign'] ? 'NorthEast' : 'East';
                    if ('bottom' === $options['vAlign']) {
                        $gravity = 'SouthEast';
                        $yAxis = $options['vOffset'] - $options['padding'];
                    }
