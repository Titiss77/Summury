...
class LocalizationSync extends BaseCommand
{
    ...
    public function run(array $params)
    {
        ...
        if (EXIT_ERROR === $this->process($optionLocale, $optionTargetLocale)) {
            return EXIT_ERROR;
        }
    ...
    private function process(string $originalLocale, string $targetLocale): int
    {
        ...
        try {
            if (!is_dir($targetLocaleDir) && !mkdir($targetLocaleDir, 0o775)) {
        ...
        foreach ($files as $originalLanguageFile) {
            if ('php' !== $originalLanguageFile->getExtension()) {
                continue;
            }
    ...
    /**
     * @param array<string, null|array<string,mixed>|string> $originalLanguageKeys
     * @param array<string, null|array<string,mixed>|string> $targetLanguageKeys
     *
     * @return array<string, null|array<string,mixed>|string>
     */
