...
final class SiteURIFactory
{
    ...
    {
        if (
            null === $this->superglobals->server('REQUEST_URI')
            || null === $this->superglobals->server('SCRIPT_NAME')
        ) {
            return '';
        }
        ...
        if (
            ...
            && 'php' === pathinfo($this->superglobals->server('SCRIPT_NAME'), PATHINFO_EXTENSION)
        ) {
    ...
    /**
     * @return null|string The current hostname. Returns null if no valid host.
     */
    private function getHost(): ?string
    {
        $httpHostPort = $this->superglobals->server('HTTP_HOST') ?? null;

        if (null !== $httpHostPort) {
    ...
    /**
     * @return null|string The valid hostname. Returns null if not valid.
     */
