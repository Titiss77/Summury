...
class Forge extends BaseForge
{
    ...
    protected function _alterTable(string $alterType, string $table, $processedFields)
    {
        ...
        $sql .= ('ADD' === $alterType) ? 'ADD ' : ' ';

        $sqls = [];

        if ('ADD' === $alterType) {
            foreach ($processedFields as $field) {
                $sqls[] = $sql.(false !== $field['_literal'] ? $field['_literal'] : $this->_processColumn($field));
            }
        ...
        foreach ($processedFields as $field) {
            ...
            if (isset($field['null']) && (false === $field['null'] || $field['null'] === ' NOT '.$this->null)) {
                $nullable = false;
            }
            $sqls[] = $sql.' ALTER COLUMN '.$this->db->escapeIdentifiers($field['name'])
    ...
    protected function _processColumn(array $processedField): string
    {
        return $this->db->escapeIdentifiers($processedField['name'])
            ...
            .' '.$processedField['type'].('text' === $processedField['type'] ? '' : $processedField['length'])
    ...
    {
        if (!empty($attributes['AUTO_INCREMENT']) && true === $attributes['AUTO_INCREMENT'] && str_contains(strtolower($field['type']), strtolower('INT'))) {
