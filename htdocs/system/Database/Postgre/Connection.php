...
class Connection extends BaseConnection
{
    ...
    public function connect(bool $persistent = false)
    {
        ...
        if (false !== $this->connID) {
            if (
                $persistent
                && PGSQL_CONNECTION_BAD === pg_connection_status($this->connID)
                && false === pg_ping($this->connID)
            ) {
            ...
            if (false === $this->setClientEncoding($this->charset)) {
                $error = pg_last_error($this->connID);
    ...
    {
        if (false === $this->connID || false === pg_ping($this->connID)) {
            $this->close();
            $this->initialize();
        }
    }
    ...
    protected function _fieldData(string $table): array
    {
        ...
        for ($i = 0, $c = count($query); $i < $c; ++$i) {
            ...
            $retVal[$i]->nullable = 'YES' === $query[$i]->is_nullable;
    ...
    {
        return 0 === pg_set_client_encoding($this->connID, $charset);
    }
     ...
     */
    private function convertDSN(): void
    {
        // Strip pgsql
